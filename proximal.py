...
bn=BatchNormalization()(hidden3)
hidden4=Dense(30,activation='relu')(bn)
...
decoder1=model.layers[-2](encoded_input)
decoder2=model.layers[-1](decoder1)

Decoder=Model(encoded_input, decoder2)
...
surf=ax.plot_trisurf(low_dim[:,0], low_dim[:,1],np.squeeze(y_test), cmap=plt.cm.coolwarm, linewidth=0.2, antialiased=True)#cmap=cm.coolwarm
fig.colorbar(surf, shrink=0.5, aspect=5)
ax.scatter(low_dimGd[:,0], low_dimGd[:,1], y_gd, c='r', marker='o')
ax.scatter(min_test_x1, min_test_x2, min_test_y, c='g', marker='^',s=20)
plt.title("True Objective")#set title
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
...
ax.plot_trisurf(X_fit[:,0], X_fit[:,1], np.squeeze(fhat), cmap=plt.cm.coolwarm, linewidth=0.2, antialiased=True)#cmap=cm.coolwarm
fig.colorbar(surf, shrink=0.5, aspect=5)
ax.scatter(low_dimGd[:,0], low_dimGd[:,1], y_gd, c='r', marker='o')
ax.scatter(min_x1, min_x2, min_y, c='g', marker='^',s=20)
plt.title("NN Estimated Objective")
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
