...
model.fit(X_train, y_train, epochs=epochs, verbose=1, validation_split=0.25, callbacks=callback)
...
surf = ax.plot_trisurf(low_dim[:, 0], low_dim[:, 1], np.squeeze(y_test), cmap=plt.cm.coolwarm, linewidth=0.2,
...
ax.plot_trisurf(X_fit[:, 0], X_fit[:, 1], np.squeeze(fhat), cmap=plt.cm.coolwarm, linewidth=0.2,
                antialiased=True)  # cmap=cm.coolwarm
fig.colorbar(surf, shrink=0.5, aspect=5)
ax.scatter(low_dimGd[:, 0], low_dimGd[:, 1], y_gd, c='r', marker='o')
