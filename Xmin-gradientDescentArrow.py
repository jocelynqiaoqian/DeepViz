...
# x returns a very big value if x_i<0 and returns 0 elsewhere
...
# calculate the objective function f(x)+g(x), where g(x)=sum(xi)
fnn = fnn + sum(X)
...
np.random.seed(12345)
X1dtest = 50 * np.random.rand(1, N_test) - 25
X2dtest = 50 * np.random.rand(1, N_test) - 25
...
y=np.asarray([[8],[2],[5]])
pre_X=[[10],[10],[10]]
gd = gradientDescent(50,A,y,pre_X,Encoder,model,0.001)
...
# # ax.scatter(np.squeeze(np.asarray(gd[:,0])), np.squeeze(np.asarray(gd[:,1])), c='r', marker='o')
# # ax.plot(np.squeeze(np.asarray(gd[:,0])), np.squeeze(np.asarray(gd[:,1])),c='r')
# plt.tricontour(triangd,np.squeeze(gd))#draw contour lines
